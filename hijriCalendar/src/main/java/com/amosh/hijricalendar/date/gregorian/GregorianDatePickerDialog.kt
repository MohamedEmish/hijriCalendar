 ...
 */
class GregorianDatePickerDialog : DialogFragment(), View.OnClickListener, DatePickerController {
    enum class Version {
        VERSION_1, VERSION_2
    }

    private var mTimezone: TimeZone = TimeZone.getDefault()
    private val mCalendar = trimToMidnight(Calendar.getInstance(timeZone))
    ...
    private var mMinYear = Calendar.getInstance().get(Calendar.YEAR)
    private var mMaxYear = Calendar.getInstance().get(Calendar.YEAR)
    ...
    private var highlightedDays: HashSet<Calendar>? = HashSet<Calendar>()
    private var selectableDays = TreeSet<Calendar>()
    private var disabledDays = HashSet<Calendar>()
    ...
    interface OnDateSetListener {
         ...
         */
        fun onDateSet(view: GregorianDatePickerDialog?, year: Int, monthOfYear: Int, dayOfMonth: Int)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        VERSION_2_FORMAT = SimpleDateFormat(DateFormat.getBestDateTimePattern(mLocale, "EEEMMMdd"), mLocale)
    ...
    override fun onCreateView(
        ...
        if (savedInstanceState != null) {
            ...
            minDate = savedInstanceState.getSerializable(KEY_MIN_DATE) as Calendar
            maxDate = savedInstanceState.getSerializable(KEY_MAX_DATE) as Calendar
            highlightedDays = savedInstanceState.getSerializable(KEY_HIGHLIGHTED_DAYS) as HashSet<Calendar>
            selectableDays = savedInstanceState.getSerializable(KEY_SELECTABLE_DAYS) as TreeSet<Calendar>
            disabledDays = savedInstanceState.getSerializable(KEY_DISABLED_DAYS) as HashSet<Calendar>
        ...
        val viewRes: Int = if (mVersion == Version.VERSION_1) R.layout.hdp_mdtp_date_picker_dialog else R.layout.hdp_mdtp_date_picker_dialog_v2
        val view: View = inflater.inflate(viewRes, container, false)
        // All options have been set at this point: round the initial selection if necessary
        setToNearestDate(mCalendar)
        mDatePickerHeaderView = view.findViewById<View>(R.id.mdtp_date_picker_header) as TextView
        mMonthAndDayView = view.findViewById<View>(R.id.mdtp_date_picker_month_and_day) as LinearLayout
        mMonthAndDayView?.setOnClickListener(this)
        mSelectedMonthTextView = view.findViewById<View>(R.id.mdtp_date_picker_month) as? TextView
        mSelectedDayTextView = view.findViewById<View>(R.id.mdtp_date_picker_day) as TextView
        mYearView = view.findViewById<View>(R.id.mdtp_date_picker_year) as TextView
        ...
        if (!mThemeDarkChanged) {
            mThemeDark = Utils.isDarkTheme(requireActivity(), mThemeDark)
        }
        val res = resources
        ...
        view.setBackgroundColor(ContextCompat.getColor(requireContext(), bgColorResource))
        mAnimator = view.findViewById<View>(R.id.mdtp_animator) as AccessibleDateAnimator
        ...
        view.findViewById<View>(R.id.mdtp_day_picker_selected_date_layout).setBackgroundColor(accentColor)
    ...
    private fun updateDisplay(announce: Boolean) {
        mYearView?.text = String.format(locale, "%2d", mCalendar[Calendar.YEAR])
        if (mVersion == Version.VERSION_1) {
            if (mDatePickerHeaderView != null) {
                if (mTitle != null) mDatePickerHeaderView?.text = mTitle!!.uppercase(locale) else {
                    mDatePickerHeaderView?.text = mCalendar.getDisplayName(Calendar.DAY_OF_WEEK, Calendar.LONG,
                        locale)?.uppercase(locale)
                }
            }
            mSelectedMonthTextView?.text = mCalendar.getDisplayName(Calendar.MONTH, Calendar.SHORT, locale)?.toString() ?: ""
            mSelectedDayTextView?.text = String.format(locale, "%2d", mCalendar[Calendar.DAY_OF_MONTH])
        }
        if (mVersion == Version.VERSION_2) {
            mSelectedDayTextView?.text = String.format(locale, VERSION_2_FORMAT!!.format(mCalendar.time))
    ...
    fun getHighlightedDays(): Array<Calendar>? {
        if (highlightedDays.isNullOrEmpty()) return null
        val output = highlightedDays!!.toTypedArray()
        Arrays.sort(output)
        return output
    }
    ...
    override fun isHighlighted(year: Int, month: Int, day: Int): Boolean {
        ...
        trimToMidnight(date)
        return highlightedDays?.contains(date) ?: false
    }
    ...
    fun setSelectableDays(selectableDays: Array<Calendar>) {
        for (selectableDay in selectableDays) trimToMidnight(selectableDay)
    ...
    fun setDisabledDays(disabledDays: Array<Calendar>) {
        for (disabledDay in disabledDays) trimToMidnight(disabledDay)
    ...
    fun getDisabledDays(): Array<Calendar>? {
        ...
        val output = disabledDays.toTypedArray()
        Arrays.sort(output)
        return output
    }
    ...
    override fun onClick(v: View) {
        tryVibrate()
        if (v.id == R.id.mdtp_date_picker_year) {
            setCurrentView(YEAR_VIEW)
        } else if (v.id == R.id.mdtp_date_picker_month_and_day) {
    ...
    private fun isBeforeMin(calendar: Calendar): Boolean {
        return minDate != null && calendar.before(minDate)
    }

    private fun isAfterMax(calendar: Calendar): Boolean {
        return maxDate != null && calendar.after(maxDate)
    }
    ...
    private fun setToNearestDate(calendar: Calendar) {
        if (!selectableDays.isEmpty()) {
            ...
            val highDistance = abs(higher.timeInMillis - calendar.timeInMillis)
            val lowDistance = abs(calendar.timeInMillis - lower!!.timeInMillis)
            when {
                lowDistance < highDistance -> calendar.timeInMillis = lower.timeInMillis
                else -> calendar.timeInMillis = higher.timeInMillis
            }
            return
        }
        if (disabledDays.isNotEmpty()) {
            val forwardDate = calendar.clone() as Calendar
            val backwardDate = calendar.clone() as Calendar
     ...
     */
    private fun trimToMidnight(calendar: Calendar): Calendar {
    ...
    private fun notifyOnDateListener() {
        if (mCallBack != null) {
            mCallBack!!.onDateSet(this@GregorianDatePickerDialog, mCalendar[Calendar.YEAR],
    ...
    companion object {
        ...
        private var YEAR_FORMAT = SimpleDateFormat("yyyy", Locale.getDefault())
        private var MONTH_FORMAT = SimpleDateFormat("MMM", Locale.getDefault())
        private var DAY_FORMAT = SimpleDateFormat("dd", Locale.getDefault())
        private var VERSION_2_FORMAT: SimpleDateFormat? = null
        var mLocale: Locale = Locale.ROOT
        ...
        fun newInstance(
            ...
            val ret = GregorianDatePickerDialog()
